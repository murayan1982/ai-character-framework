...
def _load_private_config(path: Path, *, root: Path) -> dict[str, Any]:
    ...
    if set(endpoint) != {"host", "port"}:
        raise ValueError("Private endpoint configuration contains unexpected fields.")
    ...
    normalized = {str(key).strip().casefold(): str(value).strip() for key, value in bindings.items()}
    if len(normalized) != 5 or any(not key or not value for key, value in normalized.items()):
        raise ValueError("Exactly five non-empty private hotkey bindings are required.")
    if any(len(key) > 128 or len(value) > 128 for key, value in normalized.items()):
        raise ValueError("Private hotkey selectors and names must be at most 128 characters.")
    ...
    for selector in normalized:
        ...
        if not separator or prefix not in {"expression", "emotion", "gesture"} or not selected:
            raise ValueError("Private hotkey selectors do not match the accepted five intents.")
        groups[prefix].append(selector)
    if any(len(values) != 1 for values in groups.values()):
        raise ValueError("Private hotkey bindings must contain one selector per accepted intent.")
    ...
    return {
        ...
        "bindings": normalized,
        "selector_by_intent": {key: values[0] for key, values in groups.items()},
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--authenticate-timeout-seconds", type=float, default=10.0)
...
def _print_markers(*, status: str, stage: str, values: Mapping[str, Any]) -> None:
...
def main() -> int:
    ...
    try:
        ...
        for value, label in (
            ...
            (args.authenticate_timeout_seconds, "authenticate timeout"),
        ...
        if _is_inside(evidence_root, root):
            raise ValueError("Evidence root must remain outside the repository.")
        ...
        if private["token_path"].stat().st_size > 4096:
            raise ValueError("Private authentication token file is too large.")
        token = private["token_path"].read_text(encoding="utf-8").strip()
        if not token or len(token) > 256:
            raise ValueError("Private authentication token is empty or invalid.")
        ...
        session = create_motion_session(
            ...
            vts_connect_timeout_seconds=float(args.connect_timeout_seconds),
            vts_authenticate_timeout_seconds=float(args.authenticate_timeout_seconds),
            vts_request_timeout_seconds=float(args.request_timeout_seconds),
            vts_close_timeout_seconds=float(args.close_timeout_seconds),
        )
        token = ""
        ...
        values.update(
            {
                "actual_pyvts_imported": metadata.get("provider_sdk_imported") is True,
                "actual_websocket_connected": metadata.get("connected") is True,
                "actual_vts_authenticated": metadata.get("authenticated") is True,
                "actual_model_loaded": metadata.get("model_loaded") is True,
                "actual_hotkey_inventory_loaded": metadata.get("hotkey_inventory_loaded") is True,
            }
        )
        capability_ready = all(
            (
                ...
                capability.supports_stop_motion,
                values["private_config_outside_repo"],
                values["private_token_outside_repo"],
                values["private_evidence_outside_repo"],
        ...
        for intent_name in (
            ...
            protocol_executed = result_metadata.get("provider_protocol_call_executed") is True
            ...
            observed = _observe(intent_name)
            if not observed:
                raise RuntimeError(f"{intent_name}_visual_confirmation_missing")
            values[f"{intent_name}_verified"] = True
            results[intent_name] = {
                ...
                "real_hotkey_execution_verified": bool(
                    completed and protocol_executed and hotkey_resolved and observed
                ),
        ...
        values["real_hotkey_execution_verified"] = all(
            entry["real_hotkey_execution_verified"] for entry in results.values()
        )
        values["real_motion_execution_verified"] = all(
            entry["real_motion_verified"] for entry in results.values()
        )
        values["operator_visual_confirmation_complete"] = all(
            entry["operator_visual_confirmation"] for entry in results.values()
        )
        ...
        values["bridge_thread_terminated"] = not _bridge_threads_alive()
        session = None
        ...
        run_dir = evidence_root / f"v550_vts_acceptance_{run_id}"
        run_dir.mkdir(parents=False, exist_ok=False)
        evidence_path = run_dir / "real_motion_operator_evidence.json"
        evidence = {
            ...
            "private_config_outside_repo": values["private_config_outside_repo"],
            ...
            "actual_websocket_connected": values["actual_websocket_connected"],
            "actual_vts_authenticated": values["actual_vts_authenticated"],
            "actual_model_loaded": values["actual_model_loaded"],
            "actual_hotkey_inventory_loaded": values["actual_hotkey_inventory_loaded"],
            "intent_results": results,
            "real_hotkey_execution_verified": values["real_hotkey_execution_verified"],
            "real_motion_execution_verified": values["real_motion_execution_verified"],
            "operator_visual_confirmation_complete": values["operator_visual_confirmation_complete"],
            "session_close_verified": values["session_close_verified"],
            "bridge_thread_terminated": values["bridge_thread_terminated"],
        ...
        success = all(
            (
                ...
                values["actual_model_loaded"],
                values["actual_hotkey_inventory_loaded"],
                values["expression_verified"],
                values["emotion_verified"],
                values["gesture_verified"],
                values["reset_expression_verified"],
        ...
        print("v550_vts_private_evidence_validation: run-private-verifier")
        ...
        print("v550_vts_operator_safe_message: operator acceptance did not complete")
        return 1
